...
bool PluginCore::initPluginParameters()
{
	...
	piParam = new PluginParameter(controlID::m_fSumLevel, "Sum Level", "dB", controlVariableType::kFloat, -60.000000, 12.000000, -6.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(100.00);
	piParam->setBoundVariable(&m_fSumLevel, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- continuous control: Diff Level
	piParam = new PluginParameter(controlID::m_fDiffLevel, "Diff Level", "dB", controlVariableType::kFloat, -60.000000, 12.000000, -6.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(100.00);
	piParam->setBoundVariable(&m_fDiffLevel, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- continuous control: LFF Fc
	piParam = new PluginParameter(controlID::m_fLFFFc, "LFF Fc", "Hz", controlVariableType::kFloat, 10.000000, 2000.000000, 100.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(100.00);
	piParam->setBoundVariable(&m_fLFFFc, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- continuous control: LPEQ Fc
	piParam = new PluginParameter(controlID::m_fLPEQFc, "LPEQ Fc", "Hz", controlVariableType::kFloat, 20.000000, 10000.000000, 1000.000000, taper::kVoltOctaveTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(100.00);
	piParam->setBoundVariable(&m_fLPEQFc, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- continuous control: LPEQ Gain
	piParam = new PluginParameter(controlID::m_fLPEQGain, "LPEQ Gain", "dB", controlVariableType::kFloat, -24.000000, 24.000000, 0.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(100.00);
	piParam->setBoundVariable(&m_fLPEQGain, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- continuous control: LPEQ Q
	piParam = new PluginParameter(controlID::m_fLPEQQ, "LPEQ Q", "", controlVariableType::kFloat, 0.200000, 20.000000, 0.707000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(100.00);
	piParam->setBoundVariable(&m_fLPEQQ, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- continuous control: APF Fc
	piParam = new PluginParameter(controlID::m_fAPFFc, "APF Fc", "Hz", controlVariableType::kFloat, 20.000000, 20480.000000, 20480.000000, taper::kVoltOctaveTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(100.00);
	piParam->setBoundVariable(&m_fAPFFc, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- continuous control: APF Gain
	piParam = new PluginParameter(controlID::m_fAPFGain, "APF Gain", "dB", controlVariableType::kFloat, -60.000000, 20.000000, 0.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(100.00);
	piParam->setBoundVariable(&m_fAPFGain, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- discrete control: Enable APF
	piParam = new PluginParameter(controlID::m_uAPFEnable, "Enable APF", "SWITCH OFF,SWITCH ON", "SWITCH OFF");
	piParam->setBoundVariable(&m_uAPFEnable, boundVariableType::kInt);
	piParam->setIsDiscreteSwitch(true);
	addPluginParameter(piParam);

	// --- continuous control: HSF Fc
	piParam = new PluginParameter(controlID::m_fHSFFc, "HSF Fc", "Hz", controlVariableType::kFloat, 1000.000000, 20480.000000, 5000.000000, taper::kVoltOctaveTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(100.00);
	piParam->setBoundVariable(&m_fHSFFc, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- continuous control: HSF Gain
	piParam = new PluginParameter(controlID::m_fHSFGain, "HSF Gain", "dB", controlVariableType::kFloat, -24.000000, 24.000000, 0.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(100.00);
	piParam->setBoundVariable(&m_fHSFGain, boundVariableType::kFloat);
	addPluginParameter(piParam);

	// --- discrete control: EQ Listen
	piParam = new PluginParameter(controlID::m_uEQListen, "EQ Listen", "SWITCH OFF,SWITCH ON", "SWITCH OFF");
	piParam->setBoundVariable(&m_uEQListen, boundVariableType::kInt);
	piParam->setIsDiscreteSwitch(true);
	addPluginParameter(piParam);

	// --- continuous control: Output Lvl
	piParam = new PluginParameter(controlID::m_fOutputLvl, "Output Lvl", "dB", controlVariableType::kFloat, -60.000000, 12.000000, -12.000000, taper::kLinearTaper);
	piParam->setParameterSmoothing(false);
	piParam->setSmoothingTimeMsec(100.00);
	piParam->setBoundVariable(&m_fOutputLvl, boundVariableType::kFloat);
	addPluginParameter(piParam);
	...
	// --- RAFX GUI attributes
	// --- controlID::m_fSumLevel
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483695);
	setParamAuxAttribute(controlID::m_fSumLevel, auxAttribute);

	// --- controlID::m_fDiffLevel
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483697);
	setParamAuxAttribute(controlID::m_fDiffLevel, auxAttribute);

	// --- controlID::m_fLFFFc
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483696);
	setParamAuxAttribute(controlID::m_fLFFFc, auxAttribute);

	// --- controlID::m_fLPEQFc
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483693);
	setParamAuxAttribute(controlID::m_fLPEQFc, auxAttribute);

	// --- controlID::m_fLPEQGain
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483693);
	setParamAuxAttribute(controlID::m_fLPEQGain, auxAttribute);

	// --- controlID::m_fLPEQQ
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483693);
	setParamAuxAttribute(controlID::m_fLPEQQ, auxAttribute);

	// --- controlID::m_fAPFFc
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483694);
	setParamAuxAttribute(controlID::m_fAPFFc, auxAttribute);

	// --- controlID::m_fAPFGain
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483694);
	setParamAuxAttribute(controlID::m_fAPFGain, auxAttribute);

	// --- controlID::m_uAPFEnable
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(1073741824);
	setParamAuxAttribute(controlID::m_uAPFEnable, auxAttribute);

	// --- controlID::m_fHSFFc
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483696);
	setParamAuxAttribute(controlID::m_fHSFFc, auxAttribute);

	// --- controlID::m_fHSFGain
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483696);
	setParamAuxAttribute(controlID::m_fHSFGain, auxAttribute);

	// --- controlID::m_uEQListen
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(1073741829);
	setParamAuxAttribute(controlID::m_uEQListen, auxAttribute);

	// --- controlID::m_fOutputLvl
	auxAttribute.reset(auxGUIIdentifier::guiControlData);
	auxAttribute.setUintAttribute(2147483695);
	setParamAuxAttribute(controlID::m_fOutputLvl, auxAttribute);


	// **--0xEDA5--**

	// Bind controls to Satyarth's wrapper
	kernel.push(&m_fSumLevel, boundVariableType::kFloat, controlID::m_fSumLevel);
	kernel.push(&m_fDiffLevel, boundVariableType::kFloat, controlID::m_fDiffLevel);
	kernel.push(&m_fLFFFc, boundVariableType::kFloat, controlID::m_fLFFFc);

	kernel.push(&m_fLPEQFc, boundVariableType::kFloat, controlID::m_fLPEQFc);
	kernel.push(&m_fLPEQQ, boundVariableType::kFloat, controlID::m_fLPEQQ);
	kernel.push(&m_fLPEQGain, boundVariableType::kFloat, controlID::m_fLPEQGain);

	kernel.push(&m_fAPFFc, boundVariableType::kFloat, controlID::m_fAPFFc);
	kernel.push(&m_fAPFGain, boundVariableType::kFloat, controlID::m_fAPFGain);
	kernel.push(&m_uAPFEnable, boundVariableType::kInt, controlID::m_uAPFEnable);

	kernel.push(&m_fHSFFc, boundVariableType::kFloat, controlID::m_fHSFFc);
	kernel.push(&m_fHSFGain, boundVariableType::kFloat, controlID::m_fHSFGain);

	kernel.push(&m_uEQListen, boundVariableType::kInt, controlID::m_uEQListen);

	kernel.push(&m_fOutputLvl, boundVariableType::kFloat, controlID::m_fOutputLvl);
...
bool PluginCore::initPluginPresets()
{
	...
	setPresetParameter(preset->presetParameters, controlID::m_fSumLevel, -6.000000);
	setPresetParameter(preset->presetParameters, controlID::m_fDiffLevel, -6.000000);
	setPresetParameter(preset->presetParameters, controlID::m_fLFFFc, 100.000000);
	setPresetParameter(preset->presetParameters, controlID::m_fLPEQFc, 1000.000000);
	setPresetParameter(preset->presetParameters, controlID::m_fLPEQGain, 0.000000);
	setPresetParameter(preset->presetParameters, controlID::m_fLPEQQ, 0.707000);
	setPresetParameter(preset->presetParameters, controlID::m_fAPFFc, 20480.000000);
	setPresetParameter(preset->presetParameters, controlID::m_fAPFGain, 0.000000);
	setPresetParameter(preset->presetParameters, controlID::m_uAPFEnable, -0.000000);
	setPresetParameter(preset->presetParameters, controlID::m_fHSFFc, 5000.000000);
	setPresetParameter(preset->presetParameters, controlID::m_fHSFGain, 0.000000);
	setPresetParameter(preset->presetParameters, controlID::m_uEQListen, -0.000000);
	setPresetParameter(preset->presetParameters, controlID::m_fOutputLvl, -12.000000);
	addPreset(preset);
